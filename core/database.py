...
def init_db():
    ...
    # Background run tracking — every collection/crawl/email/rescore job gets a
    # row here so the UI can show progress, history and failures. Previously all
    # of this existed only as stdout from the uvicorn process.
    conn.execute("""
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_runs_queued ON runs(queued_at DESC)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_run_logs_run ON run_logs(run_id, id)")
...
def get_outreach(
    status: str | None = None,
    search: str | None = None,
    batch: str | None = None,   # "new" | "old" | None
    limit: int = 100,
    offset: int = 0,
) -> list[dict]:
    conn = get_connection()
...
def get_last_outreach_batch_at() -> str | None:
    conn = get_connection()
    try:
        row = conn.execute(
...
def update_search_query(qid: int, query: str = None, country: str = None,
    ...
    if remote_jobs_only is not None:
        updates.append("remote_jobs_only = ?")
...
# Keep this many runs; older ones, and their logs, are pruned on each insert.
RUN_RETENTION = 200


def create_run(kind: str, trigger: str = "manual") -> int:
    from datetime import datetime
    conn = get_connection()
    try:
        cur = conn.execute(
            "INSERT INTO runs (kind, status, trigger, queued_at) VALUES (?, 'queued', ?, ?)",
            (kind, trigger, datetime.utcnow().isoformat()),
        )
...
def _prune_runs() -> None:
    """Keep the runs table bounded. Cheap enough to do on every insert."""
    conn = get_connection()
    try:
        row = conn.execute(
            "SELECT id FROM runs ORDER BY id DESC LIMIT 1 OFFSET ?", (RUN_RETENTION,)
        ).fetchone()
        if row:
            cutoff = row["id"]
            conn.execute("DELETE FROM run_logs WHERE run_id <= ?", (cutoff,))
...
def finish_run(run_id: int, status: str, stats: dict = None, error: str = "") -> None:
    import json as _json
    from datetime import datetime
    conn = get_connection()
    try:
        conn.execute(
            "UPDATE runs SET status=?, finished_at=?, stats_json=?, error=? WHERE id=?",
            (status, datetime.utcnow().isoformat(),
             _json.dumps(stats or {}), (error or "")[:2000], run_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def update_run_progress(run_id: int, current: int, total: int, label: str = "") -> None:
    conn = get_connection()
    try:
        conn.execute(
            "UPDATE runs SET progress_current=?, progress_total=?, progress_label=? WHERE id=?",
            (int(current), int(total), (label or "")[:200], run_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def insert_run_log(run_id: int, level: str, message: str) -> None:
    from datetime import datetime
    conn = get_connection()
    try:
        conn.execute(
            "INSERT INTO run_logs (run_id, ts, level, message) VALUES (?, ?, ?, ?)",
            (run_id, datetime.utcnow().isoformat(), level, (message or "")[:4000]),
        )
        conn.commit()
    finally:
        conn.close()
...
def _run_row_to_dict(r) -> dict:
    ...
    d = dict(r)
    try:
        d["stats"] = _json.loads(d.get("stats_json") or "{}")
    except Exception:
        d["stats"] = {}
...
def get_runs(limit: int = 50, kind: str = None, status: str = None) -> list[dict]:
    conn = get_connection()
    try:
...
def get_active_run() -> dict | None:
    """The run currently queued or executing, if any. Only one at a time."""
    conn = get_connection()
    try:
        r = conn.execute(
            "SELECT * FROM runs WHERE status IN ('queued','running') ORDER BY id DESC LIMIT 1"
        ).fetchone()
    finally:
        conn.close()
    return _run_row_to_dict(r) if r else None
...
def get_run_logs(run_id: int, after_id: int = 0, limit: int = 500) -> list[dict]:
    conn = get_connection()
    try:
        rows = conn.execute(
            "SELECT id, ts, level, message FROM run_logs WHERE run_id = ? AND id > ? "
...
def reap_orphaned_runs() -> int:
    """Any run still marked running at boot died with a previous process.
    Mark it failed so the UI never shows a phantom in-progress job."""
    from datetime import datetime
    conn = get_connection()
    try:
        cur = conn.execute(
            "UPDATE runs SET status='failed', finished_at=?, "
            "error='Interrupted - the application stopped while this run was in progress' "
