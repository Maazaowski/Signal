...
def default_config() -> dict:
    ...
    return {
        ...
        "location": {
            ...
            "excluded_regions": [],  # regions that mean "not for you" -> "no"
        },
        "outreach": {
            "candidate_name": "",
            "candidate_core_tech": [],
...
def _read_preset_meta(path: Path) -> dict | None:
    import yaml
    try:
...
# The default profile for a brand-new database. Previously this rebuilt a config
# dict from ~100 lines of constants in config/settings.py that duplicated, badly,
# what the preset YAML files already contain. Seeding from a preset removes the
# duplication and means there is one place to edit targeting defaults.
DEFAULT_PRESET_SLUG = "backend_python_pk_senior"


def _default_profile() -> tuple[str, str, dict]:
    """(name, description, config) for the first-run profile."""
    import yaml

    path = PROFILES_DIR / f"{DEFAULT_PRESET_SLUG}.yaml"
    if path.exists():
        try:
            with open(path, encoding="utf-8") as f:
                data = yaml.safe_load(f) or {}
            name = data.get("name") or "Default profile"
            description = data.get("description", "")
            config = {k: v for k, v in data.items() if k not in ("name", "description")}
            return name, description, validate_config(config)
        except Exception as e:
            print(f"[profile] default preset unreadable ({e}); using a blank profile",
                  flush=True)

    # Fallback: a valid, empty-but-usable profile. Better than failing to boot.
    return ("Default profile",
            "Created automatically. Edit it in Settings.",
            default_config())


def ensure_first_run_seed() -> int | None:
    ...
    try:
        ...
        if count > 0:
            return None
        ts = datetime.utcnow().isoformat()
        name, description, cfg = _default_profile()
        cur = conn.execute(
            "INSERT INTO profiles (name, description, config_json, created_at, updated_at, source) "
            "VALUES (?, ?, ?, ?, ?, ?)",
            (name, description, json.dumps(cfg), ts, ts,
             f"preset:{DEFAULT_PRESET_SLUG}"),
        )
        pid = cur.lastrowid
        conn.execute(
            "INSERT OR REPLACE INTO app_settings (key, value, updated_at) "
...
def _read_active_profile_id() -> int | None:
    conn = get_connection()
    try:
        row = conn.execute(
...
def _read_profile_row(pid: int) -> sqlite3.Row | None:
    conn = get_connection()
    try:
        return conn.execute(
...
def _load_active_profile_config() -> tuple[int | None, dict]:
    pid = _read_active_profile_id()
    if pid is None:
