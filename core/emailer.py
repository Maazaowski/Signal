...
from core.database import (
    get_unemailed_outreach,
    log_email,
    mark_outreach_emailed,
)
from core.profile import get_active_profile
from core.runlog import log  # noqa: F401  (shared seam: stdout + ring buffer + run_logs)
...
def _render_card(i: int, item: dict) -> str:
    ...
    job_url = _escape(job_url) if job_url.lower().startswith(("http://", "https://")) else "#"
...
def send_daily_digest(limit: int = None, dry_run: bool = False) -> dict:
    ...
    limit = limit or settings.get("digest_size")

    profile = get_active_profile()
    out_cfg = profile.get("outreach") or {}
    # Read at send time, so a credential saved in Settings takes effect on the
    # very next send rather than needing a restart.
    ...
    recipient = (out_cfg.get("recipient_email") or "").strip() or settings.get("recipient_email")

    if not sender or not sender_password:
        return {"error": "Email sender is not set up yet. Add it in Settings -> Email delivery."}
    if not recipient:
        return {"error": "No recipient set. Add one in Settings -> Email delivery."}
...
async def run_daily_pipeline(send: bool = True) -> dict:
    ...
    from core.runlog import progress, warn

    log("Step 1/3: Collecting jobs...")
    progress(0, 3, "Collecting jobs")
    ...
    log("Step 2/3: Generating outreach...")
    progress(1, 3, "Generating outreach")
    generated = generate_outreach_for_top_jobs()
    log(f"  Generated {generated} new outreach items")

    # The email step is gated here, not around the scheduler. A missing sender
    # must not stop the collection from running - it only skips delivery.
    email_result = {"skipped": True, "reason": "send=false"}
    if send:
        progress(2, 3, "Sending digest email")
        if not (settings.get("sender_email") and settings.get("sender_password")):
            email_result = {
                "skipped": True,
                "reason": "Email sender is not set up (Settings -> Email delivery)",
            }
            warn("Step 3/3: Email skipped - no sender configured in Settings. "
                 "Roles were still collected and intros generated.")
        else:
